export const simpleStorage: string = `pragma solidity 0.5.11;
...
contract dexBlueSettlementModule is dexBlueStorage, dexBlueEvents, dexBlueUtils, dexBlueStructs {
    ...
    function matchOrders(
        ...
        // Do maker and taker want to trade the same tokens with each other
            ...
            balances[makerOrder.sellToken][makerOrder.signee] -= trade.makerAmount - trade.makerRebate;
            // Substract maker's sold amount minus the makers rebate

            balances[takerOrder.sellToken][takerOrder.signee] -= trade.takerAmount;
            // Substract taker's sold amount
            ...
            // Give the maker his bought amount minus the fee

            balances[takerOrder.buyToken][takerOrder.signee] += trade.makerAmount - trade.takerFee;
            // Give the taker his bought amount minus the fee
            ...
            // Prevent maker order from being reused

            matched[takerOrder.hash] += trade.takerAmount;
            // Prevent taker order from being reused
            ...
            // Give the feeColletor the taker fee minus the maker rebate

            balances[makerOrder.buyToken][feeCollector] += trade.makerFee;
            // Give the feeColletor the maker fee
            ...
            // If the maker tried to block funds which he/she used in this order we have to unblock them

            blocked_for_single_sig_withdrawal[takerOrder.sellToken][takerOrder.signee] = 0;
            // If the taker tried to block funds which he/she used in this order we have to unblock them
    ...
    function matchOrderWithReserve(
        ...
        // Does the order signee has the required balances deposited
            ...
            balances[order.sellToken][order.signee] -= trade.orderAmount - trade.orderRebate;
            // Substract users's sold amount minus the makers rebate
            ...
            // executeReserveTrade checks whether the reserve deposited the funds
                ...
                // Substract reserves's sold amount
                ...
                // Give the users his bought amount minus the fee
                ...
                // Give the feeColletor the fee minus the maker rebate
                ...
                // If the user blocked funds which he/she used in this order we have to unblock them
    ...
    function batchSettleTrades(OrderInputPacked[] calldata orderInput, TradeInputPacked[] calldata tradeInput) external {
        ...
        // Check if msg.sender is an arbiter and the market is active
    ...
    function settleRingTrade(OrderInputPacked[] calldata orderInput, RingTradeInputPacked[] calldata tradeInput) external {
        ...
        for (i = 0; i < trades.length; i++) {
            ...
            if (trades[i].isReserve) {// Ring element is a reserve
                ...
                balances[order.sellToken][order.signee] -= trades[i].giveAmount - trades[i].rebate;
                // Substract sold amount minus the makers rebate from order signees balance
                ...
                // Give the order signee his bought amount minus the fee
                ...
                // Prevent order from being reused
                ...
                blocked_for_single_sig_withdrawal[order.sellToken][order.signee] = 0;
                // If the order signee tried to block funds which he/she used in this order we have to unblock them
    ...
    */
    ...
    function swap(address sell_token, uint256 sell_amount, address buy_token, uint256 min_output, uint256 deadline) external payable returns (uint256){
        ...
        // Check which reserve offers the best deal on the trade
        ...
        // Execute the swap with the best reserve
    ...
    */
    ...
    function swapWithReserve(address sell_token, uint256 sell_amount, address buy_token, uint256 min_output, address reserve, uint256 deadline) public payable returns (uint256){
        ...
        if (sell_token == address(0)) {// Caller wants to swap ETH
            ...
            // Deposit ERC20 into the reserve
        ...
        if (
            ...
            // Substract the amount from the reserves balance
            ...
            if (buy_token == address(0)) {// Is the bought asset ETH
                ...
                // Transfer the output token of the swap msg.sender
...
contract dexBlue is dexBlueStorage, dexBlueEvents, dexBlueUtils, dexBlueStructs {
    ...
    function depositEther() public payable {
        ...
        // Add the received ETH to the users balance
    ...
    function() external payable {
        ...
        if (msg.sender != wrappedEtherContract) {// ETH sends from WETH contract are handled in the depositWrappedEther() function
            ...
            depositEther();
            // Call the deposit function to credit ETH sent in this transaction
    ...
    function depositWrappedEther(uint256 amount) external {
        ...
        // Remember ETH balance before the call
        ...
        // Check whether the ETH was deposited



        balances[address(0)][msg.sender] += amount;
        // Credit the deposited eth to users balance
    ...
    function depositToken(address token, uint256 amount) external {
        ...
        // Credit the deposited token to users balance
    ...
    function multiSigWithdrawal(address token, uint256 amount, uint64 nonce, uint8 v, bytes32 r, bytes32 s) external {
        ...
        // Call multiSigSend to send funds to msg.sender
    ...
    function multiSigSend(address token, uint256 amount, uint64 nonce, uint8 v, bytes32 r, bytes32 s, address payable receiving_address) public {
        ...
        && balances[token][msg.sender] >= amount                    // Check if the user holds the required balance
            ...
            balances[token][msg.sender] -= amount;
            // Substract the withdrawn balance from the users balance
            ...
            // emit LogWithdrawal event

        } else {

            revert();
            // Revert the transaction if checks fail
    ...
    function multiSigTransfer(address token, uint256 amount, uint64 nonce, uint8 v, bytes32 r, bytes32 s, address receiving_address) external {
        ...
        && balances[token][msg.sender] >= amount                    // Check if the user holds the required balance
            ...
            balances[token][msg.sender] -= amount;
            // Substract the balance from the withdrawing account

            balances[token][receiving_address] += amount;
            // Add the balance to the receiving account
    ...
    function userSigWithdrawal(bytes32 packedInput1, bytes32 packedInput2, bytes32 r, bytes32 s) external {
        ...
        */
        ...
        && balances[token][account] >= amount                       // Check if the user holds the required tokens
            ...
            // Deduct the withdrawn tokens from the users balance

            balances[token][feeCollector] += fee;
            // Fee to cover gas costs for the withdrawal
            ...
            revert();
            // Revert the transaction is checks fail
    ...
    function blockFundsForSingleSigWithdrawal(address token, uint256 amount) external {
        ...
        if (balances[token][msg.sender] - blocked_for_single_sig_withdrawal[token][msg.sender] >= amount) {// Check if the user holds the required funds
            ...
            // Block funds for manual withdrawal

            last_blocked_timestamp[msg.sender] = block.timestamp;
            // Start waiting period

            emit LogBlockedForSingleSigWithdrawal(msg.sender, token, amount);
            // emit LogBlockedForSingleSigWithdrawal event

        } else {

            revert();
            // Revert the transaction if the user does not hold the required balance
    ...
    function initiateSingleSigWithdrawal(address token, uint256 amount) external {
        ...
        || single_sig_waiting_period == 0                                                           // or the waiting period is disabled
            ...
            balances[token][msg.sender] -= amount;
            // Substract the tokens from users balance
            ...
            if (blocked_for_single_sig_withdrawal[token][msg.sender] >= amount) {
                ...
                blocked_for_single_sig_withdrawal[token][msg.sender] = 0;
                // Substract the tokens from users blocked balance
            ...
            // emit LogSingleSigWithdrawal event

        } else {

            revert();
            // Revert the transaction if the required checks fail
    ...
    function settleTrade(OrderInputPacked calldata makerOrderInput, OrderInputPacked calldata takerOrderInput, TradeInputPacked calldata tradeInput) external {
        ...
        // delegate the call to the settlement module
    ...
    function executeReserveReserveTrade(
        ...
        // Substract the deposited amount from the taker reserve
        ...
        if (trade.takerToken != address(0)) {
            ...
            // Send the taker reserves collateral to the maker reserve
        ...
        // Substract the maker reserves's sold amount
        ...
        // Give feeColletor the taker fee

        balances[trade.takerToken][feeCollector] += trade.makerFee;
        // Give feeColletor the maker fee
    ...
    function executeReserveTrade(
        ...
        // check if the reserve delivered on the request, else revert



        return true;
        // return true if all checks are passed and the trade was executed successfully
    ...
    */
    ...
    function multiSigOrderBatchCancel(bytes32[] calldata orderHashes, uint8 v, bytes32 r, bytes32 s) external {
        ...
        if (
            ...
            for (uint256 i = 0; i < len; i++) {
                ...
                // Set the matched amount of all orders to the maximum

                emit LogOrderCanceled(orderHashes[i]);
                // emit LogOrderCanceled event
    ...
    function revokeDelegation(address delegate, uint8 v, bytes32 r, bytes32 s) external {
        ...
        // Set to 1 not 0 to prevent double delegation, which would make old signed orders valid for the new delegator
    ...
    constructor() public {
        ...
        owner = msg.sender;
        // Nominate sender to be the contract owner
    ...
    function cacheReserveAddress(address payable reserve, uint256 index, bool is_public) external {
        ...
        if (is_public) {
            ...
            // append the reserve to the reserve array
    ...
    function removePublicReserveAddress(address reserve) external {
        ...
        for (uint256 i = 0; i < public_reserve_arr.length; i++) {
            ...
            if (public_reserve_arr[i] == reserve) {
                ...
                // array order does not matter, so we just move the last element in the slot of the element we are removing
    ...
    function nominateFeeCollector(address payable collector) external {
        ...
        // Check if sender is owner and feeCollector address is not locked
    ...
    */
    ...
    function directWithdrawal(address token, uint256 amount) external returns (bool){
        ...
        if (
            ...
            balances[token][msg.sender] -= amount;
            // Substract the feeCollectors balance
            ...
            // emit LogDirectWithdrawal event
